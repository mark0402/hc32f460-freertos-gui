...
本项目是一套基于 HC32F460 的嵌入式 GUI 终端开发底座，并提供三阶段配套学习文档。

**硬件平台**：主线选用 HC32F460 国产 Cortex-M4F 芯片（200MHz、带 FPU、外设齐全），完整覆盖 GPIO、时钟树、中断、DMA、Flash 掉电保存等核心外设，并涉及"在资源受限的 MCU 上同时运行 FreeRTOS 与 GUI"这一典型工程问题，适合作为嵌入式 GUI 终端开发的实践平台。

**提供的内容**：三篇文档对应完整的学习路线，由浅入深、逐步递进：

| 阶段 | 文档 | 内容 |
|---|---|---|
| ① 裸机地基 | [`docs/HC32F460零基础实战DDL/`](docs/HC32F460零基础实战DDL/) | 基于官方 DDL 库实现 GPIO / USART / 定时器 / SPI / ADC / 中断驱动，侧重数据手册阅读与外设配置 |
| ② RTOS 内核 | [`docs/HC32F460-freeRTOS实战/`](docs/HC32F460-freeRTOS实战/) | 将 FreeRTOS V10.4.6 移植到 HC32F460，覆盖任务划分、调度、IPC（信号量 / 队列 / 事件组）、中断与 RTOS 边界、内存管理，并引出本项目的工程骨架 |
| ③ GUI 外壳 | [`docs/HC32F460-guislice实战/`](docs/HC32F460-guislice实战/) | 将 GUIslice 适配到 HC32F460 + ST7789 彩屏，覆盖驱动适配层、界面状态机、主循环融合、中文字库生成，最终交付开机动画 + 主页 + 设置页 + 串口 CLI 的完整终端 |

**能完成的实践**：通过本系列，可在同一块开发板上从裸机驱动起步，逐步完成 RTOS 移植与 GUI 适配，最终构建一个具备开机动画、实时时钟、单键设置与串口分级日志 CLI 的可运行终端。每篇文档中的概念均配有可编译、可调试、可观察的示例代码。

**可达到的目标**：掌握一套相对标准的嵌入式 GUI 终端开发方法——串口控制台负责交互与联调、GUI 负责人机界面、FreeRTOS 负责任务编排。其中的底层思想（分层设计、适配层、RTOS 边界、资源脚本生成）与具体芯片无关，可平移至 STM32、GD32 等任意 Cortex-M 平台。

除三篇主文档外，`docs/HC32F460-笔记篇/` 收录 GCC 工具链搭建、Console 组件与分层日志等专题笔记，供随时查阅。
...
- 购买链接（含资料）：<https://item.taobao.com/item.htm?id=693801792793>
